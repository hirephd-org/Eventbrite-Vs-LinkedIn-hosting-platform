# -*- coding: utf-8 -*-
"""Final -Event platform comparison.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1t3diEWkw74HCCxpf1rI4DLSdszFDb0y-
"""
...
df_no_duplicates.to_csv('report_no_duplicates.csv', index=False)
...
report_data = pd.read_csv('report_no_duplicates.csv')
report_data.head()
report_data.columns
...
! cat report_no_duplicates.csv

# reading the csv file using read_csv
# storing the data frame in variable called df
df = pd.read_csv('report_no_duplicates.csv')
...
# Assuming you have a column named 'Order Date'
date_column = 'Order Date'
...
# 'Order Date' is the column with dates
df['Order Date'] = pd.to_datetime(df['Order Date'], errors='coerce')

# Drop rows with missing or invalid dates
df = df.dropna(subset=['Order Date'])

# Replace 'specific_date' with the actual date (e.g., '2022-01-01')
specific_date = pd.to_datetime('20220709')

# Ensure both 'Order Date' column and 'specific_date' are timezone-aware (or both naive)
df['Order Date'] = pd.to_datetime(df['Order Date'], errors='coerce', utc=True).dt.tz_localize(None)
specific_date = specific_date.tz_localize(None)

# Drop rows after the specific date
df = df[df['Order Date'] <= specific_date]
